...
public class Circle : MonoBehaviour
{
    ...
    public void Set(float x, float y, float z, int a)
    {
        ...
        Appr.transform.localScale = new Vector2(2,2);
        Fore.transform.localScale = new Vector2(2,2);
        Back.transform.localScale = new Vector2(2,2);
    }
    ...
    private IEnumerator Checker()
    {
        while (true)
        {
            ...
            if (GameHandler.timer >= PosA + (GameHandler.ApprRate + 100) && !GotIt)
            {
                Remove();
